...
public class LibraryService
{
    ...
    * Register local songs and web libraries that were previously cached
    ...
    private static void registerCachedSongs()
    {
        ...
        if(REGISTER_SONGS_BETTER_SOURCES)
        {
            ...
            {
                Log.println(Log.ERROR, "[Shook-CACHE]", "Cache restore : IOException");
                e.printStackTrace();
            }
        }
    ...
    * This method is asynchronous
    ...
    /*
     * If the option is enabled, Shook will try to find a better source for all the songs added by WebService
     * Example : you added a spotify album, but deezer prior > spotify ; Shook will load that album from Deezer
     */
    ...
    static void loadAlbumArt(Album alb, String path, boolean local)
    {
        ...
        else
        {
            ...
            if(!toSave.exists())
            {
                ...
                {
                    Log.println(Log.WARN, "[Shook]", "Exception on decoding album image for album " + alb.getName() + " : " + path);
                    e.printStackTrace();
                    return;
                }
            }
